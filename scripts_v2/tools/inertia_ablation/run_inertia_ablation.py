"""Matched inertial interventions with fixed default gains and feedback timing."""
import argparse
...
OUTPUT=ROOT.parent/'23_inertia_gap'
out=OUTPUT/(args.output_name or args.phase)/f'worker_{args.worker}'
out.mkdir(parents=True,exist_ok=True)
assert not (out/'summary.json').exists(),out
runner_source=Path(__file__).read_bytes()
(out/'overrides.py').write_bytes(Path(__file__).with_name('overrides.py').read_bytes())
...
inertial_intervention=apply_overrides(env,robot,gains,P)
(out/'inertial_intervention.json').write_text(json.dumps(inertial_intervention,indent=2)+'\n')
...
for j in range(G):
    s=slice(j*P,(j+1)*P)
    assert torch.allclose(masses[s],masses[j*P:j*P+1].expand_as(masses[s]))
    assert torch.allclose(inertias[s],inertias[j*P:j*P+1].expand_as(inertias[s]))
snapshot=dict(robot_usd=cfg.scene.robot.spawn.usd_path,
    inertial_intervention=inertial_intervention,
...
relative_zero=torch.tensor([g['protocol']=='relative_zero' for g in gains],device=env.device).repeat_interleave(P)
steps=1200
...
failed_environments=torch.zeros(N,dtype=torch.bool,device=env.device)
failure_events=[]
def record_nonfinite(mask,policy_step,substep):
    newly_failed=mask&~failed_environments
    if not newly_failed.any():return
    failed_ids=torch.nonzero(newly_failed,as_tuple=True)[0]
    failure_events.append(dict(policy_step=policy_step,substep=substep,physics_samples_recorded=tindex,
        environment_ids=failed_ids.cpu().tolist(),
        case_ids=[gain_ids[int(i)//P] for i in failed_ids.cpu().tolist()],
        pose_indices=(failed_ids%P).cpu().tolist(),
        joint_positions=robot.data.joint_pos[failed_ids].cpu().tolist(),
        joint_velocities=robot.data.joint_vel[failed_ids].cpu().tolist()))
    failed_environments[newly_failed]=True
    (out/'nonfinite_failures.json').write_text(json.dumps(dict(events=failure_events,
        handling='Failure is permanent. No reset or rescue. Subsequent commanded efforts are zero for failed environments; raw trajectory samples remain recorded. Failed environments cannot pass.'),indent=2)+'\n')
for policy_step in range(100):
    ...
    desired_p=torch.where(relative_zero[:,None],current_p,desired_p)
    desired_q=torch.where(relative_zero[:,None],current_q,desired_q)
    ...
    target_residual=(arm._ee_pos_des-desired_p).abs().amax(dim=1)
    record_nonfinite(~torch.isfinite(target_residual),policy_step,-1)
    failed=torch.isfinite(target_residual)&(target_residual>=2e-6)&~failed_environments
    if failed.any():
        failed_ids=torch.nonzero(failed,as_tuple=True)[0]
        (out/'target_failure.json').write_text(json.dumps(dict(policy_step=policy_step,
            failed_environment_ids=failed_ids.cpu().tolist(),
            failed_case_ids=(failed_ids//P).cpu().tolist(),
            residual_m=target_residual[failed_ids].cpu().tolist(),
            current_position=current_p[failed_ids].cpu().tolist(),desired_position=desired_p[failed_ids].cpu().tolist(),
            reconstructed_position=arm._ee_pos_des[failed_ids].cpu().tolist(),
            joint_positions=robot.data.joint_pos[failed_ids].cpu().tolist(),joint_velocities=robot.data.joint_vel[failed_ids].cpu().tolist()),indent=2)+'\n')
        torch.save(dict(values=values[:tindex].cpu(),gain_ids=gain_ids,pose_count=P),out/'partial_trajectories.pt')
        raise RuntimeError('Target reconstruction failure saved with partial trajectories')
    for substep in range(12):
        env.action_manager.apply_action()
        if failed_environments.any():
            failed_ids=torch.nonzero(failed_environments,as_tuple=True)[0]
            robot.set_joint_effort_target(torch.zeros_like(robot.data.joint_effort_target[failed_ids]),env_ids=failed_ids)
        ...
        if policy_step==25 and substep==0:
            ...
            formula_error=float((expected-tau)[~failed_environments].abs().max());assert formula_error<1e-5
        ...
        record_nonfinite(~torch.isfinite(values[tindex]).all(dim=1),policy_step,substep)
        tindex+=1
    if policy_step%10==9:
...
finite=torch.isfinite(values).all(dim=0).all(dim=1)&~failed_environments.cpu()
...
summary=dict(phase=args.output_name or args.phase,pose_split=args.phase,pilot=args.pilot,worker=args.worker,physics_hz=120,decimation=12,action_hz=10,
    ...
    nonfinite_failure_count=int(failed_environments.sum()),nonfinite_failure_log='nonfinite_failures.json' if failure_events else None,
