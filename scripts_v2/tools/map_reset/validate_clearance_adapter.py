...
good = []
for path in sorted((geometry.ATLAS / 'pilot').glob('h*.npz')):
    with np.load(path) as data:
        chosen = data['chosen_q'][data['status'] == 4]
        if len(chosen):
            good.append(chosen[rng.choice(len(chosen), min(256, len(chosen)), replace=False)])
...
column_checker = RobotCollisionChecker(RobotCollisionCheckerCfg.load_from_config(robot_config=model,
    scene_model=cfg['column_scene_model'], collision_activation_distance=0., self_collision_activation_distance=0.))
spheres[..., 3] += cfg['column_clearance_m'] - cfg['radius_padding_m']
expected_world &= column_checker.get_collision_constraint(spheres).reshape(len(q), -1).amax(dim=1) <= 0
result = dict(configurations=len(q), self_disagreements=int((actual_self != expected_self).sum()),
              ...
              scope='Open-hand numerical equivalence with 50 mm column clearance, 1 mm other-world clearance, and retained self-pair exclusions.')
(root / '49_clearance_and_placement/sphere_adapter_validation.json').write_text(json.dumps(result, indent=2)+'\n')
