"""Plot measured/optimized joint traces, showing a held-out suffix when present."""
import argparse
import json
...
has_held_out = int(data['train_steps']) < len(data['real_q'])
baseline = None
...
for signal, unit, name in [('q', 'degrees', 'angles'), ('v', 'degrees/s', 'speeds')]:
    ...
    for j, ax in enumerate(axes.flat):
        ...
        ax.plot(t, baseline_curve, color='#999999', linewidth=1, alpha=.8,
                label='Initial parameter guess' if baseline is None else 'Unfitted half-reference model')
    ...
    axes[-1, 0].set_xlabel('Command index × nominal timestep'); axes[-1, 1].set_xlabel('Command index × nominal timestep')
    scope = (f'First {split:g} seconds optimized; remaining samples predicted without resetting'
             if has_held_out else 'All samples from this recording optimized; no held-out data')
    fig.suptitle('Thunder dynamics fit — '+name+'\n'+scope, fontsize=13)
...
ax.set_title('Training objective — lower is better' + ('; held-out data excluded' if has_held_out else ''))
