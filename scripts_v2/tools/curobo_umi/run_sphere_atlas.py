...
def prepare():
    ...
    cfg.update(version=1,robot_model=str(model),scene_model=str(ROOT/'15_table_reachability/model/lab_scene.yml'),
        x_world_m=old['broad']['x_world_m'],y_world_m=old['broad']['y_world_m'],
        height_above_table_m=(np.arange(1,31)*.02).tolist(),orientations=old['broad']['orientations'],
        ...
        modeling_notes=[
            'Full tabletop XY and uniform 2 cm height increments, identical 504 orientations at all positions.',
            ...
            'Exactly the accepted comparison padding: +1 mm per sphere; 2 mm combined self-pair padding.',
    ...
    paths=[model,Path(cfg['scene_model']),ROOT/'15_table_reachability/model/export_audit.json',
...
def main(args):
    ...
    if args.pilot:
        jobs=[jobs[i] for i in [0,251,503,7056,7307,7559,14616,15119]]
    ...
    for hi,oi in jobs:
        ...
        for start in range(0,count,cfg['batch_size']):
            ...
            clear=success&~self_bad&~world_bad
